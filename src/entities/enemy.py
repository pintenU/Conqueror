...
def _draw_goblin(surf, x, y, t, ts):
    """
    Hunched, creepy goblin — green skin, leather tunic/pants,
    stick raised in left hand, right hand dragging low,
    snarling mouth with tusks, warts, furrowed brow.
    Ears drawn before head fill so they appear attached.
    """
    s  = ts
    cx = x + s // 2
    cy = y + s // 2

    bob = int(math.sin(t * 3.2) * max(1, s // 32))
    ...
    surf.blit(sh, (cx - s // 2 + s // 8, cy + s // 3))

    # ── Clawed feet — follow leg swing ──────────────────────────────────
    fw = max(8, s // 7)
    fh = max(4, s // 14)
    foot_y = cy + s // 3 + bob
    # foot x positions driven by swing (computed again here before legs use them)
    stride_f = max(3, s // 10)
    swing_f  = math.sin(t * 7.0)
    lfoot_x_base = cx - fw // 2 - max(2, s // 16) + int(swing_f * stride_f)
    rfoot_x_base = cx + fw // 2 + max(2, s // 16) - int(swing_f * stride_f)
    # Left foot
    pygame.draw.ellipse(surf, (58, 42, 24),
                        (lfoot_x_base - fw // 2, foot_y, fw, fh))
    for fdx, fdy in [(-s // 10, s // 8), (-s // 14, s // 7), (-s // 20, s // 7)]:
        pygame.draw.line(surf, (42, 28, 14),
                         (lfoot_x_base, foot_y + fh // 2),
                         (lfoot_x_base + fdx, foot_y + fh // 2 + fdy),
                         max(1, s // 40))
    # Right foot
    pygame.draw.ellipse(surf, (58, 42, 24),
                        (rfoot_x_base - fw // 2, foot_y, fw, fh))
    for fdx, fdy in [(s // 10, s // 8), (s // 14, s // 7), (s // 20, s // 7)]:
        pygame.draw.line(surf, (42, 28, 14),
                         (rfoot_x_base, foot_y + fh // 2),
                         (rfoot_x_base + fdx, foot_y + fh // 2 + fdy),
                         max(1, s // 40))

    # ── Legs — animated walk cycle ───────────────────────────────────────
    # Reuse swing_f and stride_f computed in the feet section above
    lw       = max(5, s // 9)
    leg_top  = cy + s // 10 + bob
    stride   = stride_f
    l_swing  = int(swing_f * stride)
    r_swing  = int(-swing_f * stride)
    lfoot_x  = lfoot_x_base
    rfoot_x  = rfoot_x_base
    knee_off = max(4, s // 10)
    # Left leg: hip -> knee -> foot
    lknee_x  = cx - lw - max(1, s // 16) - knee_off // 2 + l_swing // 2
    lknee_y  = leg_top + max(8, s // 6)
    pygame.draw.line(surf, pants,
                     (cx - lw - max(1, s // 16), leg_top),
                     (lknee_x, lknee_y), lw)
    pygame.draw.line(surf, pants,
                     (lknee_x, lknee_y),
                     (lfoot_x, foot_y), lw)
    # Right leg: hip -> knee -> foot
    rknee_x  = cx + max(1, s // 16) + knee_off // 2 + r_swing // 2
    rknee_y  = leg_top + max(8, s // 6)
    pygame.draw.line(surf, pants_d,
                     (cx + max(1, s // 16), leg_top),
                     (rknee_x, rknee_y), lw)
    pygame.draw.line(surf, pants_d,
                     (rknee_x, rknee_y),
                     (rfoot_x, foot_y), lw)
    # Knee bumps at animated positions
    pygame.draw.circle(surf, pants_d, (lknee_x, lknee_y + bob), max(3, s // 18))
    pygame.draw.circle(surf, pants_d, (rknee_x, rknee_y + bob), max(3, s // 18))

    # ── Body — hunched torso ─────────────────────────────────────────────
    bw = max(14, s * 7 // 16)
    bh = max(12, s * 5 // 16)
    body_top = cy - bh // 4 + bob
    body_cx  = cx + max(1, s // 20)   # slight forward lean
    pygame.draw.ellipse(surf, tunic,
                        (body_cx - bw // 2, body_top, bw, bh))
    pygame.draw.ellipse(surf, tunic_d,
                        (body_cx - bw // 2, body_top, bw, bh), max(1, s // 40))
    # Tunic stitching
    pygame.draw.line(surf, tunic_d,
                     (body_cx - bw // 6, body_top + max(1, s // 40)),
                     (body_cx - bw // 4, body_top + bh - max(1, s // 40)),
                     max(1, s // 48))
    pygame.draw.line(surf, tunic_d,
                     (body_cx + bw // 6, body_top + max(1, s // 40)),
                     (body_cx + bw // 4, body_top + bh - max(1, s // 40)),
    ...
    pygame.draw.line(surf, (90, 56, 32),
                     (body_cx - bw // 2 + max(2, s // 32), belt_y),
                     (body_cx + bw // 2 - max(2, s // 32), belt_y),
                     max(3, s // 18))
    pygame.draw.rect(surf, (138, 112, 48),
                     (body_cx - max(4, s // 18), belt_y - max(2, s // 28),
                      max(8, s // 9), max(4, s // 18)))

    # ── Left arm — reaching forward low, weapon extended ─────────────────
    arm_w = max(3, s // 12)
    # Shoulder to elbow
    pygame.draw.line(surf, skin,
                     (body_cx - bw // 2 + max(1, s // 16), body_top + bh // 4 + bob),
                     (cx - bw // 2 - max(4, s // 8), body_top + bh // 2 + bob),
                     arm_w)
    # Elbow to hand
    lhand_x = cx - bw // 2 - max(6, s * 3 // 16)
    lhand_y = body_top + bh * 3 // 4 + bob
    pygame.draw.line(surf, skin,
                     (cx - bw // 2 - max(4, s // 8), body_top + bh // 2 + bob),
                     (lhand_x, lhand_y),
                     arm_w)
    pygame.draw.circle(surf, skin, (lhand_x, lhand_y), max(3, s // 16))
    for fdx, fdy in [(-s // 14, s // 9), (-s // 20, s // 8), (-s // 30, s // 8)]:
        pygame.draw.line(surf, darker,
                         (lhand_x, lhand_y),
                         (lhand_x + fdx, lhand_y + fdy),
                         max(1, s // 44))

    # ── Stick in left hand ───────────────────────────────────────────────
    stick_tip_x = lhand_x - max(8, s // 6)
    stick_tip_y = lhand_y - max(10, s // 5)
    pygame.draw.line(surf, (139, 94, 60),
                     (lhand_x, lhand_y),
                     (stick_tip_x, stick_tip_y),
                     max(3, s // 16))
    # Crudely sharpened tip
    pygame.draw.polygon(surf, (176, 149, 106), [
        (stick_tip_x,                      stick_tip_y),
        (stick_tip_x - max(3, s // 24),    stick_tip_y - max(5, s // 12)),
        (stick_tip_x + max(4, s // 20),    stick_tip_y - max(2, s // 24)),
    ])

    # ── Right arm — dragging behind, knuckles low ────────────────────────
    rhand_x = body_cx + bw // 2 + max(4, s // 10)
    ...
    pygame.draw.line(surf, skin,
                     (body_cx - max(1, s // 24), body_top + bob),
                     (cx - max(2, s // 20), neck_top + bob),
                     neck_w)

    # ── Head — ears drawn BEFORE head fill so outline covers the join ────
    hr      = max(8, s // 5)
    head_cx = cx - max(2, s // 20)
    head_cy = neck_top - hr + bob
    ep      = max(5, s // 10)   # ear protrusion length

    # Left ear polygon — base vertices sit ON the head circle edge
    left_ear = [
        (head_cx - hr + max(1, s // 28),  head_cy - hr // 3),   # base bottom
        (head_cx - hr - ep,               head_cy - hr - ep),    # tip
        (head_cx - hr // 2,               head_cy - hr // 2),    # base top
    ]
    pygame.draw.polygon(surf, skin, left_ear)
    pygame.draw.polygon(surf, dark, left_ear, max(1, s // 48))
    # Inner ear shade
    left_inner = [
        (head_cx - hr + max(2, s // 24),  head_cy - hr // 3 + max(1, s // 48)),
        (head_cx - hr - ep + max(3, s // 18), head_cy - hr - ep + max(3, s // 18)),
        (head_cx - hr // 2 + max(2, s // 24), head_cy - hr // 2 + max(1, s // 48)),
    ]
    pygame.draw.polygon(surf, darker, left_inner)

    # Right ear polygon
    right_ear = [
        (head_cx + hr - max(1, s // 28),  head_cy - hr // 3),
        (head_cx + hr + ep,               head_cy - hr - ep),
    ...
    pygame.draw.polygon(surf, darker, right_inner)

    # Head fill — drawn AFTER ears so it covers the ear bases
    pygame.draw.circle(surf, skin, (head_cx, head_cy), hr)
    # Head outline drawn last to hide ear-base seam
    pygame.draw.circle(surf, dark, (head_cx, head_cy), hr, max(1, s // 44))

    # ── Eyes — narrow, suspicious ────────────────────────────────────────
    eox = max(3, hr // 3)
    er  = max(2, s // 24)
    for ex, ey_off in [(head_cx - eox, 0), (head_cx + eox, -max(1, s // 48))]:
        pygame.draw.circle(surf, (26, 26, 10), (ex, head_cy + ey_off), er + 1)
        pygame.draw.circle(surf, (26, 26, 10), (ex, head_cy + ey_off), er)
        pygame.draw.circle(surf, (255, 255, 255),
                           (ex + max(1, s // 48), head_cy + ey_off - max(1, s // 48)),
                           max(1, er // 2))

    # ── Brow — deep angry V ──────────────────────────────────────────────
    brow_y = head_cy - er - max(2, s // 24)
    pygame.draw.line(surf, darker,
                     (head_cx - eox - max(2, s // 28), brow_y - max(1, s // 40)),
                     (head_cx - eox + max(2, s // 28), brow_y + max(2, s // 24)),
                     max(2, s // 28))
    pygame.draw.line(surf, darker,
                     (head_cx + eox + max(2, s // 28), brow_y - max(1, s // 40)),
                     (head_cx + eox - max(2, s // 28), brow_y + max(2, s // 24)),
    ...
    mouth_w = max(6, hr * 2 // 3)
    pygame.draw.arc(surf, (26, 10, 10),
                    (head_cx - mouth_w // 2, mouth_y - max(2, s // 28),
                     mouth_w, max(4, s // 14)),
                    0, math.pi, max(2, s // 28))
    # Teeth
    tooth_w = max(2, s // 30)
    for tx in range(head_cx - mouth_w // 2 + max(2, s // 28),
                    head_cx + mouth_w // 2 - max(2, s // 28),
                    max(3, s // 20)):
        pygame.draw.rect(surf, bone,
                         (tx, mouth_y, tooth_w, max(3, s // 20)))
    # Tusks jutting down from upper jaw
    tusk_w = max(2, s // 28)
    pygame.draw.line(surf, tusk,
                     (head_cx - mouth_w // 2 + max(1, s // 28), mouth_y),
                     (head_cx - mouth_w // 2,                   mouth_y + max(4, s // 12)),
                     tusk_w)
    pygame.draw.line(surf, tusk,
                     (head_cx + mouth_w // 2 - max(1, s // 28), mouth_y),
                     (head_cx + mouth_w // 2,                   mouth_y + max(4, s // 12)),
    ...
    # ── Warts ─────────────────────────────────────────────────────────────
    for wx, wy, wr in [
        (head_cx - hr // 2, head_cy - hr // 3, max(2, s // 36)),
        (head_cx + hr // 3, head_cy + hr // 5, max(1, s // 44)),
        (head_cx - hr // 4, head_cy + hr // 4, max(1, s // 44)),
    ]:
...
def _draw_troll(surf, x, y, t, ts):
    ...
    aw=max(5,s//10)
    pygame.draw.line(surf,col,(cx-bw//2+s//16,cy-bh//4+bob),(cx-bw//2-s//5,cy-bh//2-s//8+bob),aw)
    cl=max(3,s//16)
    ...
    pygame.draw.line(surf,col,(cx+bw//2-s//16,cy-bh//4+bob),(cx+bw//2+s//8,cy+bob),aw)
    ...
    pygame.draw.line(surf,dark,(cx-eox-er,cy-bh//2-hr-hr//4-er+bob),(cx+eox+er,cy-bh//2-hr-hr//4-er+bob),max(2,s//32))
...
def _draw_dark_mage(surf, x, y, t, ts):
    ...
    orb_col=(int(160*pulse),int(80*pulse),max(0,min(255,int(240*pulse))))
    sh = pygame.Surface((s,s//5),pygame.SRCALPHA)
    pygame.draw.ellipse(sh,(0,0,0,35),(0,0,s,s//5))
    surf.blit(sh,(cx-s//2,cy+s//3+bob//2))
    rw=max(10,s//2); rh=max(12,s//2)
    pygame.draw.polygon(surf,robe,[(cx-rw//2,cy-rh//4+bob),(cx,cy+rh//2+bob),(cx+rw//2,cy-rh//4+bob)])
    pygame.draw.polygon(surf,dark,[(cx-rw//2,cy-rh//4+bob),(cx,cy+rh//2+bob),(cx+rw//2,cy-rh//4+bob)],max(1,s//48))
    bw=max(8,s//3); bh=max(10,s//3)
    pygame.draw.ellipse(surf,robe,(cx-bw//2,cy-bh//2+bob,bw,bh))
    hr=max(6,s//4)
    pygame.draw.polygon(surf,dark,[(cx-hr,cy-hr//2+bob),(cx,cy-hr*2+bob),(cx+hr,cy-hr//2+bob)])
    ...
    pygame.draw.circle(gs,(orb_col[0],orb_col[1],orb_col[2],max(0,min(255,int(30*pulse)))),(or2*2,or2*2),or2*2)
...
def _draw_imp(surf, x, y, t, ts):
    s  = ts
    cx = x+s//2; cy = y+s//2
    bob = int(math.sin(t*4)*max(2,s//20))
    hover = int(math.sin(t*2)*max(3,s//12))
    ...
    for wx,wsign in [(cx-s//5,-1),(cx+s//5,1)]:
        ...
        pygame.draw.polygon(surf,wc,[(wx,cy-s//8+hover+bob),(wx+wsign*ww,cy-wh+hover+bob),(wx+wsign*ww//2,cy+wh//2+hover+bob)])
        pygame.draw.polygon(surf,dark,[(wx,cy-s//8+hover+bob),(wx+wsign*ww,cy-wh+hover+bob),(wx+wsign*ww//2,cy+wh//2+hover+bob)],max(1,s//48))
    tx2=cx+s//4; ty2=cy+s//8+hover+bob
    pygame.draw.line(surf,dark,(tx2,ty2),(tx2+s//6,ty2+s//8),max(2,s//24))
    pygame.draw.polygon(surf,dark,[(tx2+s//6,ty2+s//8),(tx2+s//4,ty2+s//5),(tx2+s//6+s//16,ty2+s//4)])
    ...
    for hox in [-s//8,s//8]:
        pygame.draw.polygon(surf,dark,[(cx+hox-hw//2,cy-hr+hover+bob),(cx+hox,cy-hr-hh+hover+bob),(cx+hox+hw//2,cy-hr+hover+bob)])
...
def _draw_dire_rat(surf, x, y, t, ts):
    s  = ts
    cx = x+s//2; cy = y+s//2
    bob = int(math.sin(t*5)*max(2,s//24))
    ...
    er=max(4,s//12)
    for eox2 in [-s//10,s//16]:
        pygame.draw.circle(surf,fc2,(cx-bw//3+eox2,cy-bh//4-hr+bob),er)
        pygame.draw.circle(surf,belly,(cx-bw//3+eox2,cy-bh//4-hr+bob),max(2,er-2))
...
def _draw_orc_warrior(surf, x, y, t, ts):
    ...
    for lx in [cx-s//6-lw//2,cx+s//6-lw//2]:
        ...
        pygame.draw.rect(surf,metal,(lx,cy+bob,lw,lh//2))
    ...
    pygame.draw.polygon(surf,metal,[(cx+bw//2+s//4,cy-bh//2-s//8+bob),(cx+bw//2+s//3,cy-bh//2-s//5+bob),(cx+bw//2+s//5,cy-bh//2+bob)])
    nw=max(6,s//8)
    ...
    for tox in [-s//10,s//10]:
        pygame.draw.line(surf,(220,210,190),(cx+tox,cy-bh//3-hr//2+bob),(cx+tox,cy-bh//3-hr//2+s//12+bob),tw)
...
def _draw_stone_golem(surf, x, y, t, ts):
    s  = ts
    cx = x+s//2; cy = y+s//2
    bob = int(math.sin(t*1.2)*max(1,s//32))
    ...
    pygame.draw.line(surf,light,(cx-bw//2+max(1,s//48),cy-bh//2+bob),(cx-bw//2+max(1,s//48),cy+bh//2+bob),max(2,s//32))
    ...
    pygame.draw.line(surf,crack,(cx-hw//4,cy-bh//2-hh+hh*2//3+bob),(cx+hw//4,cy-bh//2-hh+hh*2//3+bob),max(1,s//48))
...
def _draw_werewolf(surf, x, y, t, ts):
    ...
    for eox2 in [-hr//2,hr//2]:
        pygame.draw.polygon(surf,fc2,[(cx+eox2-er2//2,cy-bh//3-hr+bob),(cx+eox2,cy-bh//3-hr-er2+bob),(cx+eox2+er2//2,cy-bh//3-hr+bob)])
...
def _draw_lich(surf, x, y, t, ts):
    ...
    hover = int(math.sin(t*0.8)*max(4,s//10))
    robe=(60,20,100); dark=(40,10,70); bone=(200,195,175)
    pulse=0.5+0.5*math.sin(t*2)
    aura_col=(int(140*pulse),int(50*pulse),max(0,min(255,int(220*pulse))))
    sh=pygame.Surface((s,s//6),pygame.SRCALPHA)
    pygame.draw.ellipse(sh,(0,0,0,25),(0,0,s,s//6))
    surf.blit(sh,(cx-s//2,cy+s*2//5))
    rw=max(14,s*3//5); rh=max(16,s*3//5)
    pygame.draw.polygon(surf,robe,[(cx-rw//2,cy-rh//4+hover+bob),(cx-rw//2-rw//8,cy+rh//2+hover+bob),(cx-rw//4,cy+rh//3+hover+bob),(cx,cy+rh//2+hover+bob),(cx+rw//4,cy+rh//3+hover+bob),(cx+rw//2+rw//8,cy+rh//2+hover+bob),(cx+rw//2,cy-rh//4+hover+bob)])
    pygame.draw.polygon(surf,dark,[(cx-rw//2,cy-rh//4+hover+bob),(cx-rw//2-rw//8,cy+rh//2+hover+bob),(cx-rw//4,cy+rh//3+hover+bob),(cx,cy+rh//2+hover+bob),(cx+rw//4,cy+rh//3+hover+bob),(cx+rw//2+rw//8,cy+rh//2+hover+bob),(cx+rw//2,cy-rh//4+hover+bob)],max(1,s//48))
    aw=max(2,s//28)
    ...
    gs=pygame.Surface((or2*4,or2*4),pygame.SRCALPHA)
    pygame.draw.circle(gs,(aura_col[0],aura_col[1],aura_col[2],max(0,min(255,int(40*pulse)))),(or2*2,or2*2),or2*2)
...
def _draw_generic(surf, x, y, t, ts, color):
    ...
    pygame.draw.ellipse(surf,tuple(max(0,c-40) for c in color),(cx-bw//2,cy-bh//4+bob,bw,bh),max(1,s//48))
    hr=max(5,s//5)
...
def _draw_bone_lord(surf, x, y, t, ts):
    ...
    rw=max(18,s*3//5)
    pygame.draw.polygon(surf,armour,[(cx-rw//2,cy+bob),(cx-rw//3,cy+lh+bob),(cx,cy+lh-s//8+bob),(cx+rw//3,cy+lh+bob),(cx+rw//2,cy+bob)])
    bw=max(18,s*3//4); bh=max(14,s//2)
    ...
    for ax,sign in [(cx-bw//2,-1),(cx+bw//2,1)]:
        ...
        for ci in range(3):
            ...
            pygame.draw.line(surf,bone,(ax+sign*s//3,cy+s//8+bob),(ax+sign*s//3+int(math.cos(ca)*s//8*sign),cy+s//8+bob+int(math.sin(ca)*s//8)),max(1,s//48))
    nw=max(5,s//10)
    for i in range(3):
    ...
    for ci,ch2 in [(-hr,s//8),(-hr//2,s//5),(0,s//6),(hr//2,s//5),(hr,s//8)]:
        pygame.draw.line(surf,gold,(cx+ci,cy-bh//2-hr*2+bob-hr),(cx+ci,cy-bh//2-hr*2+bob-hr-ch2),cw)
    ...
    for ex in [cx-eox,cx+eox]:
        ...
        pygame.draw.circle(surf,(int(100*pulse),int(200*pulse),max(0,min(255,int(255*pulse)))),(ex,cy-bh//2-hr*2+bob),max(2,er-1))
    pygame.draw.arc(surf,dark,(cx-hr//2,cy-bh//2-hr*2+hr//3+bob,hr,hr//3),math.pi,2*math.pi,max(1,s//48))
...
def _draw_mountain_king(surf, x, y, t, ts):
    ...
    for rx,ry,rr in [(cx-bw//3,cy-bh//4+bob,s//10),(cx+bw//4,cy+bh//8+bob,s//12),(cx,cy-bh//3+bob,s//14)]:
    ...
    tw=max(3,s//20)
    for tox in [-s//10,s//10]:
        pygame.draw.line(surf,(230,220,195),(cx+tox,cy-bh//2-hr+hr//2+bob),(cx+tox,cy-bh//2-hr+hr+bob),tw)
    for mi in range(5):
...
def _draw_archmage(surf, x, y, t, ts):
    ...
    orb=(int(160*pulse),int(60*pulse),max(0,min(255,int(240*pulse))))
    orb2=(int(240*pulse),int(140*pulse),max(0,min(255,int(40*pulse))))
    sh=pygame.Surface((s,s//6),pygame.SRCALPHA)
    pygame.draw.ellipse(sh,(0,0,0,20),(0,0,s,s//6))
    surf.blit(sh,(cx-s//2,cy+s*2//5+s//8))
    rw=max(18,s*4//5); rh=max(18,s*3//5)
    pts=[(cx-rw//2,cy-rh//4+hover+bob),(cx-rw//2-rw//6,cy+rh//2+hover+bob),(cx-rw//4,cy+rh//3+hover+bob),(cx,cy+rh//2+hover+bob),(cx+rw//4,cy+rh//3+hover+bob),(cx+rw//2+rw//6,cy+rh//2+hover+bob),(cx+rw//2,cy-rh//4+hover+bob)]
    pygame.draw.polygon(surf,robe,pts)
    pygame.draw.polygon(surf,dark,pts,max(2,s//32))
    trim_col=(180,150,40)
    pygame.draw.line(surf,trim_col,(cx-rw//2,cy-rh//4+hover+bob),(cx-rw//2-rw//6,cy+rh//2+hover+bob),max(2,s//32))
    pygame.draw.line(surf,trim_col,(cx+rw//2,cy-rh//4+hover+bob),(cx+rw//2+rw//6,cy+rh//2+hover+bob),max(2,s//32))
    ...
    hw=max(2,s//28)
    pygame.draw.line(surf,bone,(cx-rw//2,cy-rh//8+hover+bob),(cx-rw//2-s//5,cy-rh//4+hover+bob),hw)
    hr=max(7,s//6)
    ...
    pygame.draw.polygon(surf,dark,[(hat_cx-hr,hat_base_y),(hat_cx+hr,hat_base_y),(hat_cx,hat_base_y-hr*2)])
    pygame.draw.polygon(surf,robe,[(hat_cx-hr+max(2,s//48),hat_base_y),(hat_cx+hr-max(2,s//48),hat_base_y),(hat_cx,hat_base_y-hr*2+max(2,s//48))],max(1,s//48))
    ...
    for bi in range(5):
        bx2=cx-s//10+bi*(s//20)
        pygame.draw.line(surf,bone,(bx2,face_y+max(3,s//8)),(bx2,face_y+max(6,s//4)),max(1,s//48))
...
def _draw_inferno_duke(surf, x, y, t, ts):
    ...
    for fi in range(6):
        ...
        fx2=cx+int(math.cos(fa)*s//3); fy2=cy+int(math.sin(fa)*s//3)+hover+bob
        fr=max(4,s//10)
        gs=pygame.Surface((fr*2,fr*2),pygame.SRCALPHA)
        pygame.draw.circle(gs,(int(255*pulse),int(120*pulse),0,max(0,min(255,int(40*pulse)))),(fr,fr),fr)
    ...
    for wx,wsign in [(cx-s//5,-1),(cx+s//5,1)]:
        ...
        for wi in range(3):
            ...
            wex=wx+wsign*int(math.cos(wa)*ww); wey=cy-wh//2+int(math.sin(wa)*wh)+hover+bob
            pygame.draw.line(surf,dark,(wx,cy+hover+bob),(wex,wey),max(2,s//24))
        pygame.draw.polygon(surf,wc,[(wx,cy+hover+bob),(wx+wsign*ww,cy-wh+hover+bob),(wx+wsign*ww*3//4,cy+wh//3+hover+bob)])
        pygame.draw.polygon(surf,dark,[(wx,cy+hover+bob),(wx+wsign*ww,cy-wh+hover+bob),(wx+wsign*ww*3//4,cy+wh//3+hover+bob)],max(1,s//48))
    pygame.draw.line(surf,dark,(cx+s//5,cy+s//8+hover+bob),(cx+s//3,cy+s//4+hover+bob),max(3,s//16))
    pygame.draw.polygon(surf,fire,[(cx+s//3,cy+s//4+hover+bob),(cx+s//3+s//8,cy+s//5+hover+bob),(cx+s//3+s//12,cy+s//3+hover+bob)])
    bw=max(14,s//2); bh=max(14,s//2)
    pygame.draw.ellipse(surf,rc,(cx-bw//2,cy-bh//4+hover+bob,bw,bh))
    pygame.draw.ellipse(surf,dark,(cx-bw//2,cy-bh//4+hover+bob,bw,bh),max(2,s//32))
    aw=max(3,s//12)
    for ax,sign in [(cx-bw//2,-1),(cx+bw//2,1)]:
        pygame.draw.line(surf,rc,(ax,cy-bh//8+hover+bob),(ax+sign*s//4,cy+hover+bob),aw)
        for ci in range(3):
            ca=math.radians(-30+ci*30)
            pygame.draw.line(surf,dark,(ax+sign*s//4,cy+hover+bob),(ax+sign*s//4+int(math.cos(ca)*s//10*sign),cy+hover+bob+int(math.sin(ca)*s//10)),max(1,s//48))
    hr=max(8,s//5)
    ...
    for hox,hsign in [(-hr,-1),(hr,1)]:
        pygame.draw.arc(surf,dark,(cx+hox-s//8,cy-bh//4-hr*2-s//8+hover+bob,s//6,s//6),math.radians(180+hsign*90),math.radians(270+hsign*90),hw)
    ...
    for fi in range(5):
        ...
        fhx=cx+int(math.cos(fa)*hr); fhy=cy-bh//4-hr*2+int(math.sin(fa)*hr//3)+hover+bob
        fh2=max(5,s//10)+int(pulse*s//20)
        pygame.draw.polygon(surf,fire,[(fhx-max(2,s//32),fhy),(fhx,fhy-fh2),(fhx+max(2,s//32),fhy)])
...
def _draw_rat_king(surf, x, y, t, ts):
    ...
    for ti in range(4):
        ...
        tx2=cx+s//4+int(math.cos(ta)*s//4); ty2=cy+s//8+int(math.sin(ta)*s//6)+bob
    ...
    for ci in [-hr+cw,-hr//2+cw//2,hr//2-cw]:
...
def _draw_warchief_grommak(surf, x, y, t, ts):
    ...
    cw=max(16,s*3//4)
    pygame.draw.polygon(surf,(150,20,20),[(cx-cw//2,cy-s//3+bob),(cx+cw//2,cy-s//3+bob),(cx+cw//3,cy+s//3+bob),(cx-cw//3,cy+s//3+bob)])
    ...
    tw=max(3,s//20)
    for tox in [-s//10,s//10]:
        pygame.draw.line(surf,(230,218,195),(cx+tox,cy-bh//2-hr+hr//2+bob),(cx+tox,cy-bh//2-hr+hr+bob),tw)
...
def _draw_ancient_colossus(surf, x, y, t, ts):
    s  = ts
    cx = x+s//2; cy = y+s//2
    bob = int(math.sin(t*0.8)*max(1,s//40))
    ...
    for ri in range(4):
        ...
        rx2=cx+int(math.cos(ra)*s//4); ry2=cy+int(math.sin(ra)*s//5)+bob
        gs2=pygame.Surface((max(4,s//10),max(4,s//10)),pygame.SRCALPHA)
        gs2.fill((int(glow[0]*pulse),int(glow[1]*pulse),int(glow[2]*pulse),max(0,min(255,int(60*pulse)))))
    ...
    for ex in [cx-eox,cx+eox]:
        ...
        pygame.draw.ellipse(gs3,(ec_col[0],ec_col[1],ec_col[2],max(0,min(255,int(50*pulse)))),(0,0,er*3,er*3))
        surf.blit(gs3,(ex-er*3//2,cy-bh//2-hh+hh//3+bob-er),special_flags=pygame.BLEND_RGBA_ADD)
    pygame.draw.line(surf,crack,(cx-hw//3,cy-bh//2-hh+bob),(cx+hw//4,cy-bh//2+bob),max(2,s//32))
...
def _draw_the_alpha(surf, x, y, t, ts):
    ...
    for lx,sign in [(cx-s//6,-1),(cx+s//6,1)]:
        ...
        for ci in range(3):
            ...
            pygame.draw.line(surf,dark,(lx+sign*s//7,cy+s//3+s//14+bob),(lx+sign*s//7+int(math.cos(ca)*s//10*sign),cy+s//3+s//14+bob+int(math.sin(ca)*s//10)),max(1,s//48))
    bw=max(18,s*3//4); bh=max(16,s*3//5)
    pygame.draw.ellipse(surf,fc2,(cx-bw//2,cy-bh//3+bob,bw,bh))
    pygame.draw.ellipse(surf,dark,(cx-bw//2,cy-bh//3+bob,bw,bh),max(2,s//32))
    for fi in range(8):
        fa=math.radians(fi*22.5)
        fx2=cx+int(math.cos(fa)*bw//3); fy2=cy-bh//4+int(math.sin(fa)*bh//4)+bob
    ...
    for ax,sign,adx in [(cx-bw//2,cx-bw//2-s//3,-1),(cx+bw//2,cx+bw//2+s//3,1)]:
        ...
        for ci in range(4):
            ...
            pygame.draw.line(surf,dark,(adx,cy-s//8+bob),(adx+int(math.cos(ca)*s//6*sign),cy-s//8+bob+int(math.sin(ca)*s//6)),max(2,s//28))
    hr=max(10,s//4)
    pygame.draw.circle(surf,fc2,(cx,cy-bh//3-hr+bob),hr)
    pygame.draw.circle(surf,dark,(cx,cy-bh//3-hr+bob),hr,max(2,s//32))
    ...
    pygame.draw.line(surf,blood,(cx-hr//2,cy-bh//3-hr+hr//3+bob),(cx+hr//3,cy-bh//3-hr-hr//3+bob),max(2,s//32))
    er2=max(7,s//9)
    for eox2 in [-hr//2,hr//2]:
        pygame.draw.polygon(surf,fc2,[(cx+eox2-er2//2,cy-bh//3-hr+bob),(cx+eox2,cy-bh//3-hr-er2+bob),(cx+eox2+er2//2,cy-bh//3-hr+bob)])
        pygame.draw.polygon(surf,fur,[(cx+eox2-er2//3,cy-bh//3-hr+bob),(cx+eox2,cy-bh//3-hr-er2+er2//3+bob),(cx+eox2+er2//3,cy-bh//3-hr+bob)])
    er=max(3,s//14); eox=max(4,hr//3)
    for ex in [cx-eox,cx+eox]:
...
def _draw_lich_king(surf, x, y, t, ts):
    ...
    soul=(max(0,min(255,int(100*pulse))),max(0,min(255,int(30*pulse))),max(0,min(255,int(200*pulse))))
    ...
    gs4.fill((soul[0],soul[1],soul[2],max(0,min(255,int(100*pulse)))))
    ...
    for wi in range(5):
        ...
        wx2=cx+int(math.cos(wa)*s*2//5); wy2=cy+int(math.sin(wa)*s//3)+hover+bob
        gs5=pygame.Surface((max(6,s//10),max(6,s//10)),pygame.SRCALPHA)
        gs5.fill((soul[0],soul[1],soul[2],max(0,min(255,int(40*pulse)))))
        surf.blit(gs5,(wx2,wy2),special_flags=pygame.BLEND_RGBA_ADD)
    rw=max(20,s*4//5); rh=max(20,s*4//5)
    tatter_pts=[(cx-rw//2,cy-rh//4+hover+bob),(cx-rw//2-rw//5,cy+rh//2+hover+bob),(cx-rw//3,cy+rh//3+hover+bob),(cx-rw//6,cy+rh//2+rh//8+hover+bob),(cx,cy+rh//2+hover+bob),(cx+rw//6,cy+rh//2+rh//8+hover+bob),(cx+rw//3,cy+rh//3+hover+bob),(cx+rw//2+rw//5,cy+rh//2+hover+bob),(cx+rw//2,cy-rh//4+hover+bob)]
    ...
    aw=max(2,s//24)
    pygame.draw.line(surf,bone,(cx-rw//2,cy-rh//8+hover+bob),(cx-rw//2-s//5,cy-rh//2+hover+bob),aw)
    pygame.draw.line(surf,bone,(cx-rw//2-s//5,cy-rh//2+hover+bob),(cx-rw//2-s//4,cy-rh//3+hover+bob),aw)
    or3=max(6,s//10)
    ...
    pygame.draw.circle(gs6,(soul[0],soul[1],soul[2],max(0,min(255,int(60*pulse)))),(or3*2,or3*2),or3*2)
    ...
    for ex in [cx-eox,cx+eox]:
        ...
        pygame.draw.circle(gs7,(soul[0],soul[1],soul[2],max(0,min(255,int(40*pulse)))),(er*2,er*2),er*2)
        surf.blit(gs7,(ex-er*2,skull_y-er*2),special_flags=pygame.BLEND_RGBA_ADD)
    jaw_w=max(10,hr); jaw_h=max(5,hr//3)
    pygame.draw.arc(surf,dark,(cx-jaw_w//2,skull_y+hr//3,jaw_w,jaw_h),math.pi,2*math.pi,max(2,s//32))
    tw2=max(1,s//48)
